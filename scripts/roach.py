...
def secret_errors():
 ...
 return errs
def out_of_scope_changes(c):
 base=c.get("started_head")
 if not base or not commit_exists(base):return []
 changed=[x for x in git_changed(base,"HEAD") if not x.startswith(".roach/")]
 patterns=relevant_files(c)
 return sorted(x for x in changed if not (x in patterns or glob_any(x,patterns)))

def all_project_errors(require_complete=False):
 s=state();errs=ledger_errors()+contradiction_errors()+architecture_errors()+secret_errors()
 active={r["id"] for r in reqs() if r["status"]=="active"};covered=set();satisfied=set()
 for c in s["checkpoints"]:
  if c["status"]!="superseded":covered.update(c["requirements"])
  if c["status"]=="sealed":satisfied.update(c["requirements"])
...
def checkpoint_errors(s,c):
 ...
 if c["gates"].get("behavior")=="passed":
  errs += [c["id"]+": "+x for x in objective_receipt_valid(c)]
...
def start_cmd(a):
 s=state();c=cp(s,a.id);c["started_head"]=head();transition(c,"implementing");s["current_checkpoint"]=c["id"];put_state(s);event("CheckpointStarted",{"checkpoint":c["id"],"started_head":c["started_head"]});print("started",c["id"])
...
def verify_project_cmd(a):
 errs=all_project_errors(require_complete=a.complete)
 out={"valid":not errs,"complete_mode":a.complete,"protocol":PROTOCOL,"head":head(),"errors":errs}
...
def product_cmd(a):
 if a.action=="export":
  lines=[f"# {state()['project']} Product Requirements","",f"Generated by Roach Loop {PROTOCOL}. Structured source: .roach/requirements.json",""]
  for r in reqs():
   lines += [f"## {r['id']} — {r['statement']}",f"- Status: {r['status']}",f"- Kind: {r['kind']}",f"- Priority: {r['priority']}"]
   if r.get("acceptance"):
    lines += ["- Acceptance:"]+[f"  - {x}" for x in r["acceptance"]]
   if r.get("conflicts"):lines += ["- Conflicts: "+", ".join(r["conflicts"])]
   lines.append("")
  path=Path(a.path or "PRODUCT.md")
  if not path.is_absolute():path=root()/path
  path.write_text("\n".join(lines)+"\n");event("ProductSpecExported",{"path":str(path.relative_to(root())) if path.is_relative_to(root()) else str(path),"sha256":digest_bytes(path.read_bytes())});print(path)
 else:
  path=Path(a.path or "PRODUCT.md")
  if not path.is_absolute():path=root()/path
  if not path.exists():die("product document missing")
  expected=[]
  for r in reqs():expected.append(r["id"])
  text=path.read_text(errors="ignore");missing=[x for x in expected if x not in text]
  json_or_print({"valid":not missing,"missing_requirement_ids":missing,"path":str(path)},a)
  if missing:raise SystemExit(1)

def dependency_cmd(a):
 xs=load(rp("external.json"),[])
 if a.action=="add":
  if not a.id or not a.source:die("dependency add requires --id and --source")
  ...
  x=next((z for z in xs if z.get("kind")=="dependency" and z.get("id")==a.id),None)
  if not x:die("unknown dependency")
  if not x.get("verification"):die("dependency has no verification command")
  r=run(x["verification"]);x["last_check"]={"at":now(),"head":head(),"exit_code":r.returncode,"stdout_sha256":digest_text(redact(r.stdout)),"stderr_sha256":digest_text(redact(r.stderr))}
  x["status"]="verified" if r.returncode==0 else "failed";save(rp("external.json"),xs);event("ExternalDependencyChecked",{"id":x["id"],**x["last_check"]});json_or_print(x,a)
...
def status_cmd(a):
 ...
 active={r["id"] for r in reqs() if r["status"]=="active"};satisfied=set()
 for cc in s["checkpoints"]:
  if cc["status"]=="sealed":satisfied.update(cc["requirements"])
 out={"project":s["project"],"profile":s["profile"],"protocol":PROTOCOL,"head":head(),"requirements":{"active":len(active),"sealed_satisfied":len(active&satisfied),"remaining":sorted(active-satisfied)},"checkpoints":rows,"open_findings":[x for x in load(rp("findings.json"),[]) if x["status"]=="open"]}
 json_or_print(out,a)
...
def parser():
 ...
 q=sp.add_parser("product");q.add_argument("action",choices=["export","check"]);q.add_argument("--path");jout(q);q.set_defaults(fn=product_cmd)
 q=sp.add_parser("dependency");q.add_argument("action",choices=["add","list","check"]);q.add_argument("--id");q.add_argument("--source");q.add_argument("--verification");jout(q);q.set_defaults(fn=dependency_cmd)
 for name,fn in [("status",status_cmd),("doctor",doctor_cmd)]:
  q=sp.add_parser(name);jout(q);q.set_defaults(fn=fn)
 q=sp.add_parser("verify-project");q.add_argument("--complete",action="store_true");jout(q);q.set_defaults(fn=verify_project_cmd)
